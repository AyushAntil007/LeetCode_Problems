// class Solution {
// public:
// void decrement(vector<int>&nums,int left,int right,int val){
//     for(int j=left;j<=right;j++){
//         if(nums[j]>=val)nums[j]-=val;
//         else{
//             int sub=val;
//             while(nums[j]<sub){
//                 sub--;
...
//     }
// }
// bool checkZeroArr(vector<int>&nums){
...
//             int right=it[1];
//             int val=it[2];
//             if(checkZeroArr(nums))return count;
//             decrement(nums,left,right,val);
//             count++;
            
//         }
//         return checkZeroArr(nums) ? count : -1;
//     }
// };
